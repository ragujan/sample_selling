...
class UserData extends Db
{
    ...
    public function getCustomerPurchasedHistory($customerId): array
    {
        $query = "SELECT * FROM `customer_purchase_history` INNER JOIN `samples` 
        ON `samples`.`sampleID` = `customer_purchase_history`.`sampleID` 
        WHERE `CustomerID` = ? ORDER BY unique_id ASC ";
    ...
    public function getCustomerPurchasedHistoryLimit($customerId, $from,$resuls_per_page): array
    {
        $query = "SELECT * FROM `customer_purchase_history` INNER JOIN `samples` 
        ON `samples`.`sampleID` = `customer_purchase_history`.`sampleID` 
        WHERE `CustomerID` = ? ORDER BY unique_id ASC LIMIT $from,$resuls_per_page ";
        $statement = $this->connect()->prepare($query);
        $statement->execute([$customerId]);
