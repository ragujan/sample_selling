...
if (isset($_POST["current_page_number"]) && isset($_POST["sub_sample_id"])) {
    ...
    if ($subsampletypenumber == 0 || $subsampletypenumber == "ALL") {
        // if the sub sample type number is zero which means 
        $valueforBTN = 0;
       
        $max_page_count = $query_object->sampleTypePagesMidi($DefaultSampleTypeNumber);
        ...
        $results_per_page= $query_object->sampleTypeMidi($DefaultSampleTypeNumber, $current_page_number * $exactResultsPerPage);
        ...
        $max_page_count = $query_object->subSampleTypePagesMidi($subsampletypenumber);
        if ($current_page_number >= $max_page_count) {
            $current_page_number = $max_page_count;
        } else if ($current_page_number <= 0) {
            $current_page_number = 0;
        }
        $results_per_page= $query_object->subSampleTypeMidi($subsampletypenumber,$current_page_number * $exactResultsPerPage);
        $totalCount = $query_object->returnTotalCount();
        $allowedPages = ceil($totalCount / $exactResultsPerPage);
    }
...
echo $htmlContentObject->view_midi_samples($results_per_page, $allowedPages, $current_page_number, $valueforBTN, $pageName,$jsMethodName);
